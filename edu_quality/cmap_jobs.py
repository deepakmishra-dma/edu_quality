...
def generate_mention_html(base_url, user_id, message, name):
	mention_html = '<div class="ql-editor read-mode"><p>'
	mention_html += (
...
def compare_time(time_str, minute_difference):
	try:
		...
		frappe.log_error("Error comparing time for PTM Scheduler", frappe.get_traceback())
		return False
...
def get_user_id_of_instructor(teacher_id):
	...
	if user_id:
		...
		frappe.log_error(f"User ID not found for instructor {teacher_id}")
		return None
...
def notify_teacher_before_one_hour_job():
	...
	ptms = frappe.get_all(
		...
		fields=["name", "slot", "subject", "teacher", "division", "gmeet_link"],
	)
	notifi_added = []
...
def get_upcoming_online_ptm_links(student_id):
	...
	if student_division:
		...
		ptm_scheduler_list2 = frappe.get_all("PTM Scheduler", filters=filterss2, fields=["*"])
		past_ptms = False
		...
		if len(ptm_scheduler_list) > 0:
			...
			ptm_scheduler_list = [
				item for item in ptm_scheduler_list if item.get("datetime") >= datetime.datetime.now()
			]
...
def get_list_of_students_from_division_list(division_list, student_group):
	...
	students_list = frappe.db.sql(sql, {"li": tuple(division_list), "group": student_group}, as_dict=1)
...
def handle_notify(time_data, time_inwords, already_notified):
	...
	for i in time_data:
		...
		if not already_notified.get(calculated_key):
			students_lists = get_list_of_students_from_division_list([i.get("division")], i.get("group"))
			notification_handler([i.get("student") for i in students_lists], time_inwords)
