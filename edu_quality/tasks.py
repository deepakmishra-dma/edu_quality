...
def create_payment_request_before_due_date():
    ...
    for fee_schedule in fee_schedules:
        before_days = frappe.get_value(
            "Fee Schedule", fee_schedule.name, "create_payment_request_before"
        )
        fee_list = frappe.get_all(
            "Fees", {"fee_schedule": fee_schedule.name, "workflow_state": "Approved"}
        )
        for fee in fee_list:
...
def get_datetime_range():
    ...
    last_day_7pm = (
        current_time.replace(hour=19, minute=0, second=0, microsecond=0)
        - timedelta(days=1)
    ).replace(tzinfo=None)
...
def send_bulk_notification_cmap_to_guardian():
    current_academic_year = frappe.db.get_value(
        "Academic Year", filters={"custom_current_academic_year": 1}
    )
