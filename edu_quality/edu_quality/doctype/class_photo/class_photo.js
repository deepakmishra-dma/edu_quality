async function folderExists(parent, newFolder) {
  ...
  try {
    let googleRes = await fetch(
      `/api/method/edu_quality.edu_quality.doctype.class_photo.class_photo.check_folder_in_drive`,
      {
        method: "POST",
            ...
            try {
              const images = await nativeInterface.execute(
                "openWebViewCamera",
                {
                  ...
                  backgroundMode: true,
                  endpoint:
                    "edu_quality.edu_quality.doctype.class_photo.class_photo.move_existing_and_upload_to_drive",
                  parameters: {
                    method: "POST",
