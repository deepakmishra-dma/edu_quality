...
class GoogleDriveSettings(Document):
	def get_access_token(self):
		if not self.refresh_token:
			...
			raise frappe.ValidationError(f"Click on {button_label} to generate Refresh Token.")
...
def authorize_access(reauthorize=False, code=None):
	oauth_code = frappe.db.get_single_value("Google Drive Settings", "auth_code") if not code else code
	oauth_obj = GoogleOAuth("drive")
...
def upload_file_to_drive(file_url, folder_name):
	...
	try:
		media = MediaFileUpload(get_absolute_path(file_url), mimetype="image/jpeg", resumable=True)
