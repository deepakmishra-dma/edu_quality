...
from frappe.query_builder.functions import GROUP_CONCAT, Cast, Concat, Count
...
class CMAP(Document):
	def name_func(self):
		course_short_code = frappe.db.get_value("Course", self.subject, "custom_short_code")
		course_doc = frappe.get_doc("Course", self.subject)
		class_sortcode = frappe.db.get_value("Class Type", self.get("class"), "short_code")
...
def get_cmap_list(academic_year, program, subject, unit, from_date=None, end_date=None):
	...
	final_query = (
		...
		.on((item_detail_qb.parent == cmap_query.name) & (item_detail_qb.parenttype == "CMAP"))
