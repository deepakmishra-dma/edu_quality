...
function updateNoteQuery(cur_frm, res, row_name) {
  ...
  if (res == "EMPTY_IT") {
    return groups.forEach((array) => {
      getNoteQuery(cur_frm, array[0], array[1], [" "], row_name);
    });
  }
...
async function setupNotesColumns(cur_frm) {
  const EXISTING_ROWS_IN_CHILD_TABLE =
    cur_frm.fields_dict["products"].grid.grid_rows;

  for (row in EXISTING_ROWS_IN_CHILD_TABLE) {
    const item = frappe.model.get_value(
      "Item Detail",
      EXISTING_ROWS_IN_CHILD_TABLE[row].doc.name,
      "item"
    );
...
async function getProduct(id) {
  ...
  const res = await fetch(
    `/api/resource/Item/${id}?fields=["custom_additional_material"]`,
    {
      headers: headers,
    }
  );
...
async function checkNotes(type, frm, materialType) {
  const broadcastItems = frm.fields_dict["products"].grid.data?.map(
    (item) => item[type]
  );
  const headers = new Headers();
  headers.append("X-Frappe-CSRF-Token", frappe.csrf_token);
  frappe.call({
    method:
      "edu_quality.edu_quality.doctype.cmap.cmap.check_if_note_added_unique",
    args: {
...
frappe.ui.form.on("CMAP", {
  refresh(frm) {
    frm.set_query("academic_year", function () {
      return {
        query:
          "edu_quality.edu_quality.doctype.cmap.cmap.get_current_and_next_year",
      };
    });
    getNotes(frm);
    cur_frm.fields_dict["products"].grid.get_field("item_group").get_query =
      function (doc, cdt, dn) {
        let d = locals[cdt][dn];
        return {
          filters: {
    ...
    cur_frm.fields_dict["products"].grid.get_field("item").get_query =
      function (doc, cdt, dn) {
        let d = locals[cdt][dn];
        return {
          filters: {
    ...
    cur_frm.fields_dict["products"].grid.get_field("chapter").get_query =
      function (doc, cdt, dn) {
        let d = locals[cdt][dn];
        return {
          filters: {
    ...
    cur_frm.fields_dict["table_vwbr"].grid.get_field("division").get_query =
      function (doc, cdt, dn) {
        let d = locals[cdt][dn];
