...
const handleTemplateBroadcast = async (members, message) => {
  ...
  try {
    const messageCreatedRes = await fetch(
      "/api/method/edu_quality.api.broadcast_whatsapp.message",
      {
        headers: headers,
        method: "POST",
...
const getFieldsInLead = async () => {
  ...
  try {
    const response = await fetch(
      "/api/method/edu_quality.public.py.utils.generate_fields_map?docName=Lead",
      {
        headers: headers,
        method: "GET",
      }
  ...
  refresh: function (frm) {
    frm.add_custom_button(__("Broadcast Message"), async function () {
      const templates = await (
        await fetch("/api/resource/WhatsApp Template?limit=0")
      ).json();
      const mappedTemplates = templates?.data?.map((template) => {
      ...
      const whatsappTemplate = new frappe.ui.Dialog({
        ...
        primary_action: async function (values) {
          try {
            const templateReactDialog =
              document.createElement("whatsapp-template");
            ...
            templateReactDialog.docVariables =
              docVariables?.message?.array || [];
            ...
            templateReactDialog.sendFallback = async (_, message) => {
              try {
                const res = await handleTemplateBroadcast(
                  frm.doc.group_members,
                  message
                );
            ...
            templateDialog.fields_dict.render_html.$wrapper.html(
              templateReactDialog
            );
