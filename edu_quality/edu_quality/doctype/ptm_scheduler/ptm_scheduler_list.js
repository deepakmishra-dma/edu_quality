frappe.listview_settings["PTM Scheduler"] = {
  onload: function (listview) {
    ...
    listview.page.add_action_item(
      __('<i class="fa fa-video-camera"></i> Generate Gmeet Link'),
      function () {
                ...
                frappe.prompt(
                  [
                    ...
                    },
                    {
                      ...
                      fieldtype: "Check",
                      label: "Re-Generate (if Present Gmeet Link)",
                    },
                    {
                      fieldname: "imporsonate_user",
                      fieldtype: "Link",
                      label: "Select Gmeet Owner",
                      options: "User",
                      reqd: 1,
                    },
                  ],
                  function (values) {
                    ...
                    generate_gmeet_links(
                      items,
                      values.summary,
                      values.imporsonate_user,
                      values.regenerate
                    );
                  },
                  "Enter Details for Gmeet Link",
                  "Generate"
                );
              } else {
...
function importCSVData(file) {
  frappe.call({
    method:
      "edu_quality.edu_quality.doctype.ptm_scheduler.ptm_scheduler.import_ptm_schedule_from_url",
    args: { url: file },
    callback: function (response) {
      if (response.message.status === "success") {
...
function generate_gmeet_links(items, summary, imporsonate_user, regenerate) {
  ...
  frappe.call({
    method:
      "edu_quality.edu_quality.doctype.ptm_scheduler.ptm_scheduler.generate_meeting_function",
    args: {
      items,
      summary,
      imporsonate_user,
