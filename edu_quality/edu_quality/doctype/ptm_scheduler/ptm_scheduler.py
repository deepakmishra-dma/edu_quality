...
from frappe.utils.data import getdate,get_datetime
...
def generate_meeeting(summary,description,start_time,imporsonate_user):
	try:
		...
		meet_url = auth.create_meet(summary=summary,description=description, start_time=get_datetime(start_time), duration_minutes=24*60*6, imporsonate_user=imporsonate_user)
		return meet_url
	except Exception as e:
		raise e
...
def generate_meeting_function(items, summary, imporsonate_user, regenerate=False):
	try:
		...
		ptm_schedules_list = frappe.get_all("PTM Scheduler", filters=filters,
											fields=['name', 'teacher', 'division', 'gmeet_link', 'date'])
		...
		current_start_date_ac = frappe.db.get_value('Academic Year',{'custom_current_academic_year':1},'year_start_date')
		for div in division_list:
			gmeet_link = generate_meeeting(summary=summary,description ='PTM Meeting Summary',start_time=current_start_date_ac,
