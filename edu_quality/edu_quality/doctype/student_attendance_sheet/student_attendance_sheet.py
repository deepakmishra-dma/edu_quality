...
def get_months():
	months = []
	for i in range(1, 13):
		months.append({"value": calendar.month_name[i], "label": calendar.month_name[i]})
	return months
			...
			& (
				(
					...
					& (
						...
						| ((event_qb.ends_on <= all_day_end_date) & (event_qb.all_day == 1))
					)
				)
				| ((event_qb.starts_on <= start_date) & (event_qb.ends_on >= start_date))
			)
...
def get_data(month_name, academic_year, program, division):
	...
	if attendance_data:
		filtered_data = [
			entry for entry in attendance_data if entry["date"].year == year and entry["date"].month == month
		]
...
def submit_attendance(**data):
	...
	for entry in attendance_entries:
		...
		if not attendance_entry.get("status"):
			...
			attendance_entry.status = "Present" if status not in ["A", "S"] else "Absent"
	...
	for date in unique_dates:
		...
		marked_students = {entry["student"] for entry in attendance_entries if entry["date"] == date}
