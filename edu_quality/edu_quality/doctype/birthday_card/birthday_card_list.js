...
frappe.listview_settings["Birthday Card"] = {
  refresh: function (listview) {
    listview.page
      .add_inner_button(__("Print Birthday Cards"), async function () {
        const dialog = new frappe.ui.Dialog({
          ...
          primary_action: async function (values) {
            ...
            frappe.show_progress(
              ...
              25,
              100,
              "Please wait, the birthday cards are being generated..."
            );

            try {
              const response = await fetch(
                `/api/method/edu_quality.edu_quality.doctype.birthday_card.birthday_card.print_birthday_cards`,
                {
                  method: "POST",
                  headers: headers,
              ...
              console.error(
                "An error occurred while trying to print birthday cards:",
                e
              );
            } finally {
...
async function handleFileResponse(file, values) {
  try {
    ...
    const fromMonthName = from_date.toLocaleString("default", {
      month: "long",
    });
    ...
    if (message === false) {
      ...
      showMessageDialog(
        `No Birthday Card Found for birthdates between ${fromMonthName} ${from_day} and ${toMonthName} ${to_day}`
      );
      return;
    }
  } catch (e) {
