...
def get_web_form(hash):
	if frappe.db.exists("Student Applicant", {"form_hash": hash}):
		...
		if frappe.db.exists(
			"User Permission", {"allow": "Student Applicant", "for_value": student_applicant}
		):
			user = frappe.db.get_value(
				"User Permission", {"allow": "Student Applicant", "for_value": student_applicant}, "user"
			)
...
def set_guardian_permissions(doc):
	for guardian in doc.guardians:
		...
		if not frappe.db.exists(
			"User Permission", {"user": user, "allow": "Student Applicant", "for_value": doc.name}
		):
	...
	try:
		...
		for guardian in doc.guardians:
			...
			if guardian.relation == "Father":
				...
				name = (
					doc.guardian_first_name + " " + doc.guardian_last_name if doc.guardian_last_name else ""
				)
				if name:
					guardian_doc.guardian_name = name
	...
	try:
		for component in doc.components:
			...
			if discount_name and "referral" in discount_name.lower():
				...
				update_payment_plan_after_discount(
					doc, discount_amount, apply_discount=True, dis={"type": "Referral"}
				)
				if not is_paid:
	...
	try:
		for component in doc.components:
			...
			if amount > referral_amount and referral_amount != 0:
				...
				update_total_discount_in_fees(doc.name)
				update_payment_plan_after_discount(
					doc, referral_amount, apply_discount=True, dis={"type": "Referral"}
				)
				if not is_paid:
					update_payment_request_after_discount(doc)
				doc.update_split()
