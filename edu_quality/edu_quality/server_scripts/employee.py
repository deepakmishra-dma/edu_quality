...
from edu_quality.common.utils.auth import remove_user_permissions, set_user_permissions
...
def after_insert(doc, method=None):
	...
	try:
		...
		frappe.log_error(
			"Error while sending communication mail to the user",
...
def create_employee_google_user(email_key, first_name, last_name, recovery_mail, org_unit_path=None):
	try:
		...
		if not existing_user:
			recovery_mail = (
				(str(recovery_mail) or str(google_service_doc.default_recovery_email)).strip().lower()
			)
...
def migrate_employee_data(employee, new_employee):
	...
	try:
		...
		frappe.log_error(
			err_msg, frappe.get_traceback(), reference_doctype="Employee", reference_name=employee.name
		)
...
def transfer_cmap_data(employee, new_employee):
	...
	filters = {"teacher": old_instructor, "parenttype": "CMAP", "real_date": ["is", "not set"]}
...
def send_exiting_employee_mail(employee):
	...
	try:
		...
		frappe.log_error(
			"Error while sending communication mail to the Ex-Employee",
			frappe.get_traceback(),
		)
