...
from edu_quality.public.py.discount import add_discount, get_all_discounts, remove_discount
	...
	try:
		if doctype == "Fees":
			...
			if doc.outstanding_amount == 0:
				frappe.throw("Cannot Change Payment Plan As the Fee Advance is already Paid!")
				return

			components, amount = get_components(doc.fee_structure, percent, doc.is_rte)
			doc.payment_plan = payment_plan
			doc.due_date = frappe.get_value(
				"Payment Schedule", {"payment_term": doc.payment_term, "parent": payment_plan}, "due_date"
			)
			doc.amount = amount
...
def remove_payment_plan_discount(doc, custom_payment_plan=0):
	discount_configs = frappe.get_all(
		...
		for discount_config in discount_configs:
			if discount_config.fee_category == component.fees_category:
				remove_discount(
					...
					update_payment_request=True,
					custom_payment_plan=custom_payment_plan,
				)
					...
					if dis == "Referral":
						for schedule in payment_plan.payment_schedule:
							if schedule.outstanding != 0:
								discounts[schedule.payment_term] = {dis: discount[dis]}
					else:
						for schedule in payment_plan.payment_schedule:
							discounts[schedule.payment_term] = {
								dis: {
									"discount_amount": flt(
										discount[dis]["discount_amount"] * schedule.invoice_portion / 100, 2
									)
								}
							}
	return discounts
	...
	for i, ps in enumerate(payment_plan.payment_schedule):
		...
		if i == 0:
			if referral_amount > 0:
				...
				breakup = {"Referral": {"discount_amount": flt(referral_amount, 2)}}
		doc.append(
			"payment_schedule",
			{
