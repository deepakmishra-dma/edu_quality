...
def get_one(name):
	...
	query = frappe.qb.from_(QBTicket).select(QBTicket.star).where(QBTicket.name == name).limit(1)
...
def get_history(ticket: str):
	...
	history = (
		...
		.select(QBActivity.name, QBActivity.action, QBActivity.owner, QBActivity.creation)
...
def after_insert(doc, method=None):
	...
	if students:
		for student_data in students:
			...
			fees_record = frappe.get_all(
				"Fees",
				filters={"student_reference_number": student_reference_number, "program": student_class},
				fields=["name", "fee_schedule", "fee_structure", "payment_plan"],
			)
			if fees_record:
				fees_info.append(
	...
	guardians = frappe.get_all(
		"Guardian",
		filters={"email_address": doc.raised_by},
	...
	if guardians:
		for guardian_data in guardians:
			...
			if student_guardians:
				matching_students = frappe.get_all(
					"Student",
					filters={"name": ("in", [sg["parent"] for sg in student_guardians])},
					fields=[
						"name",
						"first_name",
						"school",
			...
			if matching_students:
				for student_data in matching_students:
					...
					fees_record = frappe.get_all(
						"Fees",
						filters={
							"student_reference_number": student_reference_number,
							"program": student_class,
						},
						fields=["name", "fee_schedule", "fee_structure", "payment_plan"],
					)
					...
					if payment_entries:
						...
						for entry in payment_entries:
							payment_entry_url = frappe.utils.get_url_to_form(
								"Payment Entry", entry.get("name")
							)
							payment_entry_info.append(
								f"\n<br><br><b>Payment Entry ID:{entry.get('name')}<b>, URL:<a href='{payment_entry_url}'>Link</a><br><br>"
							)
	...
	previous_tickets = frappe.get_all(
		"HD Ticket", filters={"raised_by": doc.raised_by}, fields=["name", "subject"]
	)
	if previous_tickets:
		...
		frappe.db.set_value(
			"HD Ticket", doc.name, "custom_previous_ticket_details", "\n".join(previous_tickets_info)
		)
		text = text + "\n\n".join("<br>\nprevious_tickets_info")
	else:
	...
	full_text = "\n\n".join(
		students_info + fees_info + payment_schedule_info + payment_entry_info + previous_tickets_info
	)
	if full_text:
		make(
			doctype="HD Ticket",
