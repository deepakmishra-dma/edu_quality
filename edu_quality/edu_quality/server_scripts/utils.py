...
def set_user_permission(user, doctype, docname):
	if frappe.db.exists("User Permission", {"user": user, "allow": doctype, "for_value": docname}):
		return
	frappe.get_doc(
		{"doctype": "User Permission", "user": user, "allow": doctype, "for_value": docname}
	).insert(ignore_permissions=True)


def set_form_user(doctype, docname):
	form_map = {"Student Applicant": "walnut-school-student-application", "CBSE LOC": "cbse-confirmation"}
