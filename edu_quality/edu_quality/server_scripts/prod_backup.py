...
def get_production_backups():
    #only execute on uat
    url = frappe.utils.get_url()
    if "uat" not in url:
        return
        
    #connect and get download links
    production = RemoteFrappeSite("https://erp.walnutedu.in","sankara@hybrowlabs.com","Walnut@12345")
    production.get_backups()
    config = production.backup_links['database']

    #download backup
    try:
        r = requests.get(config,stream=True,allow_redirects=True)
    ...
    if os.path.exists("database" + yesterday + ".sql.gz"):
        os.remove("database" + yesterday + ".sql.gz")

    #restore
    restore = subprocess.Popen(["bash","-c","bench --site uat restore database.sql.gz"],stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    output, error = restore.communicate()
    restore.wait()

    #migrate
    migrate = subprocess.Popen(["bash","-c","bench migrate"],stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    output, error = migrate.communicate()
    migrate.wait()

    #reset admin password
    admin = subprocess.Popen(["bash","-c","bench --site uat set-admin-password Walnut@12345"],stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    output, error = admin.communicate()
    admin.wait()
...
def enqueue_restoration():
    """
    Enqueue restoration of production database 
    """
    frappe.enqueue(
        get_production_backups,
        queue="long",
        timeout=10000,
    )
