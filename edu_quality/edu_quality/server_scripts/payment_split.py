...
def generate_split_payment(fees, update=0):
	...
	if fees.doctype == "Fees":
		...
		if case == 1:
			split_payment["Deposit"], company_wise["Deposit"], cw = get_split(
				fees, 100, case, apply_deposit=0
			)
			component_wise["Deposit"] = {"due_date": None, "breakup": cw}
			case = 0
		...
		for schedule in fees.payment_schedule:
			...
			if case == 2:
				if "deposit" in schedule.description.lower():
					apply_deposit = 1
			split_payment[schedule.payment_term], company_wise[schedule.payment_term], cw = get_split(
				fees, schedule, case, apply_deposit=apply_deposit
			)
		...
		component_wise[term] = {"due_date": str(fees.due_date), "breakup": components, "is_deposit": False}
		...
		for component in fees.components:
			...
			if case != 1:
				if schedule.discount_breakup:
					...
					component_breakup = (
						json.loads(component.discount_breakup) if component.discount_breakup else {}
					)
					if schedule_breakup:
			...
			paid_to = paid_to = frappe.get_value(
				"Company", component.custom_company, "default_easebuzz_account"
			)
			paid_from = frappe.db.get_value("Company", component.custom_company, "default_receivable_account")
			cost_center = frappe.get_value("Cost Center", {"company": component.custom_company}, ["name"])
