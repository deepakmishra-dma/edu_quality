...
def on_amend(self):
	...
	def update_progress():
		frappe.publish_progress(processed / total * 100, title="Recreating Assessment Results")
...
def name_func(assessment_plan_doc):
	assessment_plan_doc = (
		json.loads(assessment_plan_doc) if isinstance(assessment_plan_doc, str) else assessment_plan_doc
	)
	...
	if (
		...
		textbook_short = frappe.get_doc("Textbook", assessment_plan_doc.get("custom_textbook")).get(
			"short_code"
		)
	textbook_short = "ALL"
...
def check_for_duplicates(assessment_plan_doc):
	...
	for criteria in assessment_plan_doc.assessment_criteria:
		criteria_name = f"{criteria.get('assessment_criteria')}-{criteria.get('custom_exam_type')}"
		if criteria_name not in dup_cr_hash:
			dup_cr_hash[criteria_name] = True
		elif criteria_name in dup_cr_hash and assessment_plan_doc.custom_is_descriptive == 0:
			frappe.throw(
...
def get_questions(paper):
	...
	question_order_hash = {
		question.question: question.order for question in exam_paper.questions if question.selected
	}
