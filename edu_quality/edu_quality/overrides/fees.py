...
class CustomFees(Fees):
	...
	def create_payment_request(self, payment_term):
		if not payment_term:
			...
			"Payment Request", {"reference_name": self.name, "payment_term": payment_term, "docstatus": 1}
		):
	...
	def remove_discount_entry(self, company, amount):
		receivable_account, discount_account = frappe.db.get_value(
			"Company", company, ["default_receivable_account", "default_discount_account"]
		)
		entries = []
		debit_filter = {
			"voucher_type": self.doctype,
	...
	def get_company_splits(self):
		try:
			...
			for component in self.components:
				receivable_account, sales, cost_center, deposit_account = frappe.db.get_value(
					"Company",
...
def update_fee_components(doc):
	for component in doc.components:
		school = frappe.get_value(
			"Fee Component", {"fees_category": component.fees_category, "parent": doc.fee_structure}, "school"
		)
...
def get_fee_advance_entries(fees):
	fee_advance = frappe.db.get_value(
		"Fee Advance",
		{
			"student": fees.student,
			"docstatus": 1,
			"outstanding_amount": 0,
			"next_program": fees.program,
	...
	gl_entries = frappe.db.get_all(
		"GL Entry",
		filters={"voucher_type": "Fee Advance", "voucher_no": fee_advance, "is_cancelled": 0},
		fields=[
			"account",
			"debit",
			"credit",
	...
	for entry in gl_entries:
		liability_account, income_account, cost_center = frappe.db.get_value(
			"Company", entry.company, ["default_liability_account", "default_income_account", "cost_center"]
		)
...
def update_componant(doc, fee_advance):
	if fee_advance:
		...
		for item1, item2 in zip(doc.components, fee_advance.components, strict=False):
