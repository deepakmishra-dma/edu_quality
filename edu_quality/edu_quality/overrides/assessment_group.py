...
class CustomAssessmentGroup(AssessmentGroup):
	def autoname(self):
		short_acad_year = extract_year_from_academic_year_name(self.custom_academic_year)
		...
		self.name = (
			f"{self.assessment_group_name} {short_acad_year} - {school_pref}{class_type.get('short_code')}"
		)
	...
	def create_class_topper(self):
		...
		top_3_toppers, rest_toppers = divide_toppers(all_group_results, topper_percentage)
	...
	def create_subject_toppers(self):
		...
		for subject in subject_wise_result:
			...
			for result in results:
				if result.get("max_score"):
					result["percentage"] = (result.get("score") / result.get("max_score")) * 100
				else:
					result["percentage"] = 0
		...
		for subject in subject_wise_result:
			...
			top_3_toppers, rest_toppers = divide_toppers(sorted_combined_result, topper_percentage)
	...
	def delete_topper_events(self):
		assessment_group = self.get("name")
		all_group_res = frappe.db.get_all("Assessment Group Result", {"assessment_group": assessment_group})
		...
		unique_event = set(event_participant.get("parent") for event_participant in event_participants)

		all_event_details = frappe.db.get_all("Event Detail", filters={"event": ["in", unique_event]})
	...
	def get_group_class_rank(self, as_map=True):
		...
		query = (
			...
			.select(
				...
				Rank().over().orderby(assess_gr_qb.combined_percentage, order=Order.desc).as_("rank"),
			)
		)
	...
	def get_student_with_online_submission(self):
		...
		query = (
			frappe.qb.from_(assess_res_qb)
			.where((assess_res_qb.docstatus == 1) & (assess_res_qb.assessment_group == self.name))
...
def generate_group_hash():
	group_data = frappe.db.get_all("Student Group", fields=["name", "student_group_name"])
...
def generate_subject_hash():
	...
	return {subject.get("name"): subject.get("custom_short_code") for subject in group_data}
...
def create_wiki_page_for_toppers(assessment_group, topper_results, page_name, mode="class", title=""):
	...
	wiki_space = frappe.db.get_value("Assessment Group", assessment_group, "custom_wiki_space")

	new_page = create_wiki_page(page_name, content, generate_wiki_route(wiki_space, page_name))
...
def get_event_subject_name(
	...
	short_acad_year = extract_year_from_academic_year_name(assessment_group_doc.custom_academic_year)
				...
				for idx, row in enumerate(parent_row, start=1):
					try:
						...
						if name:
							...
							current_group.custom_is_final_exam_class_photo_required = is_final_photo_req
							current_group.custom_create_topper_event = create_topper
							current_group.custom_create_topper_for_class = create_topper_class
							current_group.custom_create_topper_for_division = create_topper_division
							current_group.custom_create_topper_for_subject = create_topper_subject
							current_group.custom_topper_percentage = topper_percentage or 0
							...
							current_group.custom_config_subject_type = config_subject_type
							current_group.custom_config_subject_name = config_subject_name
							current_group.custom_config_subject_textbook_used = config_subject_textbook_used

							current_group.custom_process_passing = process_pass_or_fail or 0
							current_group.custom_passing_percentage = passing_percentage or 0
	...
	for div in divs:
		...
		if not name:
			...
			assess_plan.custom_textbook = (
				"ALL" if str(config_subject_textbook_used).lower() == "all" else config_subject_textbook_used
			)
			...
			for criteria in criterias:
				...
				assess_plan_cr.custom_include_in_ranking = int(criteria.get("custom_include_in_ranking"))
				assess_plan_cr.custom_question = criteria.get("custom_question") or None
				assess_plan_cr.custom_question = criteria.get("custom_parent_question") or None
				assess_plan_cr.save()
