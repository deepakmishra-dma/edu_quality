      ...
      beforeSubmit(submission, next) {
        if (!validatePaymentData(submission.data)) {
          ...
          frappe
            .call({
              method:
                "edu_quality.edu_quality.page.payment_plan_creatio.payment_plan_creation.create_payment_plans",
              args: submission,
            })
            .then((r) => {
              if (r.message == true) {
...
function validatePaymentData(paymentData) {
  ...
  for (const term of paymentData.paymentTerms) {
    ...
    if (classPaymentData[paymentClass].dueDates.size > 0) {
      let previous_due_date = Array.from(
        classPaymentData[paymentClass].dueDates
      ).pop();
      if (dueDate < previous_due_date) {
        frappe.throw(
          `Due Date cannot be less than previous due date for ${paymentTerm}`
        );
      }
    }
