...
def create_payment_plans(**kwargs):
	try:
		...
		for class_name, class_terms in class_payment_terms.items():
			fee_structure_name = frappe.get_value(
				"Fee Structure",
				{"academic_year": academic_year, "school": school, "program": class_name},
				"name",
			)
			...
			existing_payment_plan = frappe.get_all(
				"Payment Plan",
				{"academic_year": academic_year, "school": school, "program": class_name},
				["name"],
			)
			...
			if existing_payment_plan:
				for plan in existing_payment_plan:
					...
					if len(existing_schedule) == len(class_terms):
						...
						is_identical = all(
							term["paymentTerm"] == existing_term.payment_term
							and term["invoicePortion"] == existing_term.invoice_portion
							for term, existing_term in zip(class_terms, existing_schedule, strict=False)
						)
						if is_identical:
							frappe.throw(
								"Identical Payment Plan already exists for this class " + str(class_name)
							)
							return False
