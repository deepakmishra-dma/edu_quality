...
function onLoad() {
  ...
  filtersRef = make_fieldgroup(el, [
    ...
    },
    {
      ...
      options: "",
      description:
        "Enter Ref Nos Separated by , to process only the specified ones",
      onchange: () => {
    ...
    },
    {
      ...
      options: "",
      description:
        "Doesn't cancel assessment result and  only recaclulate assessment group result",
    },
  ]);
...
function processResult() {
  frappe.confirm(
    "Are you sure you want to process the result for the selected filters",
    () => {},
    () => {
...
async function processResult() {
  ...
  if (!acad_year || !school || !program || !assess_group) {
    frappe.throw(
      __("Academic Year or School or Program or Exam Group is required")
    );
    return;
  }
  frappe.confirm(
    "If there are submitted results they will be cancelled and new ones will be created, Are you sure you want to proceed?",
    async () => {
