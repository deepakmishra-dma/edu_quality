...
def get_cmap(**filters):
	...
	division_query = (
		...
		.select(division_table.student_group_name, division_table.name.as_("division_name"))
	)
...
def get_cmap_assignments_data(data):
	...
	query = frappe.qb.from_(cmap_assig_table).where(cmap_assig_table.parent.isin(cmap)).select("*")
