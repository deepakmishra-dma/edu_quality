...
function changeCmapDataBulk(fieldGroup) {
  ...
  Object.keys(cmapData).forEach((key) =>
    cmapData[key].forEach((values, index) => {
...
function addBulkEdit() {
  if (cmapDataPresent()) globalPage.add_inner_button("Bulk Edit", editMode);
}
...
async function setupDataTable() {
  ...
  const teachersData = await frappe.call({
    method:
      "edu_quality.edu_quality.page.cmap_assignment_tool.cmap_assignment_tool.get_teachers",
    args: filters,
  });
    ...
    Object.keys(data).forEach((val) => {
      data[val].forEach((row, index) => {
        const row_html = createRow(
          val,
          row.products && row.products.join(","),
          row.division_name,
          row.teacher,
          row.plan_date,
          "",
...
function createRow(
  ...
  if (first_row)
	...
	<td>${division}</td>
	<td class="teacher-cell">${createSelect(
    teachersData,
    teacher,
    index,
    period_no
...
function createSelect(selectData, value, index, key) {
  return `<select data-index="${index}" data-key=${key} ><option></option>${selectData.map(
    (el) =>
      `<option ${el.name === value ? `selected="selected"` : ""} value="${
        el.name
      }">${el.name}</option>`
  )}</select>`;
}
...
async function saveAssignment() {
  ...
  await frappe.call({
    method:
      "edu_quality.edu_quality.page.cmap_assignment_tool.cmap_assignment_tool.update_assignment",
    args: {
      filters: filters,
      cmap_data: cmapData,
    },
