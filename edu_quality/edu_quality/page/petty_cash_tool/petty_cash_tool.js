...
frappe.pages["petty-cash-tool"].on_page_load = function (wrapper) {
  ...
  Formio.createForm(document.getElementById("formio"), formJson, {}).then(
    async (form) => {
      FormInstance = form;
...
const autofilledForm = async (form) => {
  ...
  const { message: school } = await frappe.db.get_value(
    "School",
...
const handleFormSubmit = (form, submission) => {
  frappe.call({
    method:
      "edu_quality.edu_quality.page.petty_cash_tool.petty_cash_tool.petty_cash_submit",
    args: {
    ...
    callback: (r) => {
      if (r.message.success) {
        ...
        showMessage(
          "Failed to submit Petty Cash, Please contact the administrator",
          "Error",
          "red"
        );
      }
...
async function handleSchoolChange(event, value) {
  ...
  const { message: schoolInstitution } = await frappe.db.get_value(
    "School",
    value.name,
    "institution"
  );
...
async function handleCompanyChange(event, value) {
  ...
  if (!value.name) {
    ["accountHead", "school", "type"].forEach((field) =>
      updateField(field, "")
    );
