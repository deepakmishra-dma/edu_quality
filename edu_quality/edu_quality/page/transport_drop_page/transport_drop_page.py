...
def get_transport_data(**filters):
	...
	query = (
		...
		.on((student_table.name == attendance_entry.student) & (attendance_entry.date == today))
...
def calculate_status(transport_data, absentees_hash):
	for student in transport_data:
		...
		elif "absent" in str(drop_status) or (status_type and status_type.lower() == "absent"):
...
def update_qr(acad=None, ref=None, school=None):
	...
	mark_entry(student, "onboard", "Onboard marked with qrcode scan on drop transport page")
