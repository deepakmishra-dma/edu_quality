...
from frappe.utils import getdate, parse_json, today
...
def get_school_fields_sum(row):
	...
	return sum([row.get(field, 0) + row.get(f"extra_{field}", 0) for field in school_fields_to_sum])
...
def transform_data(program_enrollments, CMAPS, class_filter, products_max_data, products_rec_data):
	...
	for i in CMAPS:
		for school in school_fields:
			...
			if i.get("product_code") in products_rec_hash:
				i["received_from_printer_on"] = products_rec_hash[i.get("product_code")].get("received_date")
				i["received_by"] = products_rec_hash[i.get("product_code")].get("received_by")

			i[school.get("fieldname")] = converted_dict.get(f'{i.get("class")}-{school.get("label")}', 0) or 0
...
def get_data_from_queries(filters=None):
	...
	cmap_query = (
		...
		.where(
			...
			& (
				(cmap.plan_date[filters.get("start_plan_date") : filters.get("end_plan_date")])
...
@frappe.whitelist()
def create_purchase_order(rows, supplier="Printer", academic_year=None, class_name=None, unit=None):
...
def append_items(purchase_order, row, school_field):
	...
	if not school_doc.get("warehouse"):
		frappe.throw(f"Error Creating Purchase Order, Warehouse for school {school_doc.name} doesn't exist")
