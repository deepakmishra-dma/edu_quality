...
function addBulkEdit(report) {
	report.page.add_inner_button(__("Bulk Edit Quantity"), async () => {
		...
		const extraFields = await frappe.call({
			"method": "edu_quality.edu_quality.report.cmap_print.cmap_print.get_extra_fields",
		...
		const options = extraFields.message.map((extraField) => ({ "label": extraField.label, value: extraField.fieldname }))
		const d = new frappe.ui.Dialog({
			...
			primary_action: (values) => {
				const column = frappe.query_report.datatable.datamanager.columns.find((col) => col.fieldname == values.field_name)

				indexes.forEach((index) => {
					const row = frappe.query_report.data[index]
					console.log(row)
					frappe.query_report.data[index] = {
						...row, [values.field_name]: values.field_value
					}
				})

				indexes.forEach((index, rowIndex) => {
					frappe.query_report.datatable.bodyRenderer.cellmanager.updateCell(column.colIndex, index, values.field_value, true)
				})
...
function getFilters() {
	const filters = frappe.query_report.filters.reduce((acc, curr) => {
		return { ...acc, [curr.fieldname]: frappe.query_report.get_filter_value(curr.fieldname) }
	}, {});
	return filters
}