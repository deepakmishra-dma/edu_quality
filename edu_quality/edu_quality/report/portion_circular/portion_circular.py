...
from frappe.query_builder.functions import GROUP_CONCAT, Concat, Count
...
class GroupConcat(DistinctOptionFunction):
	def __init__(self, term: Field, order_field=None, order=Order.asc, sep=",", alias=None):
...
def get_data(filters):
	...
	item_groups = frappe.db.get_all("Item Group", filters={"custom_hide_in_walsh": 0}, fields=["name"])
	...
	find_filtered_cmap = (
		...
		GroupConcat(item_detail_table.item, order_field=item_detail_table.item).as_("item_names"),
		GroupConcat(
			Case()
			.when(
