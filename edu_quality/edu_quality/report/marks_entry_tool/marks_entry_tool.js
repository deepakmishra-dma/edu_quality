...
function changeMarksData(
  ...
  const inputEl = document.querySelector(
    `input[column='${columnId}'][data-rowindex='${rowIndex}']`
  );
...
function writeMarks(rowIndex, columnId, value, columnIndex) {
  ...
  if (!ref_no && !student_name) {
    ...
    if (!criteriasChanged[ref_no]) criteriasChanged[ref_no] = {};
...
function getNextElement(rowIndex, colIndex) {
  ...
  nextEl = document.querySelector(
    `input[data-rowindex="${nextRowIndex}"][data-colindex="${nextColIndex}"]`
  );
...
function createInputElement(value, column, row, docstatus, onlineAssess) {
  ...
  } data-docstatus="${docstatus || 0}" data-colindex="${
    column.colIndex - 1
  }" class="${classes.join(" ")}" column="${column.fieldname}" data-rowindex="${
    row && row[0] && row[0].rowIndex
  }" max="${column.maximum_score}" maximum-score="${
    column.maximum_score
  }" value="${value}" oninput="changeMarksData(this, '${column.id}', '${
    row[0]?.rowIndex
  }', '${column.maximum_score}','${column.scoring_type}',${
    column.colIndex
  })" onfocusout="updateCell(this, '${column.id}', '${row[0]?.rowIndex}', '${
    column.maximum_score
  }','${column.scoring_type}',${column.colIndex})" />`;
}
...
async function saveCall(autoSave = false) {
  ...
  frappe.query_report.filters.forEach((filter) => {
    filters[filter.fieldname] = frappe.query_report.get_filter_value(
      filter.fieldname
    );
  });
...
function sendMarks(assessment_group, division) {
  frappe.call({
    method:
      "edu_quality.edu_quality.report.marks_entry_tool.marks_entry_tool.send_marks",
    args: {
...
const cancelResult = debounce(async (ref_nos) => {
  ...
  await frappe.call({
    method:
      "edu_quality.edu_quality.report.marks_entry_tool.marks_entry_tool.cancel_result_rows",
    args: {
      ref_nos: ref_nos,
      filters: filters,
    },
...
function checkFilePermission(file) {
  frappe.call({
    method:
      "edu_quality.edu_quality.doctype.ptm_scheduler.ptm_scheduler.check_file_permission",
    args: { file_url: file },
...
function importCSVData(file) {
  ...
  frappe.call({
    method:
      "edu_quality.edu_quality.report.marks_entry_tool.marks_entry_tool.import_mark_entry_csv",
    ...
    callback: function (response) {
      if (response.message.status === "success") {
        ...
        frappe.msgprint(
          '<div style="color: red;">Import failed: <br>' +
            response.message.message +
            "</div>"
        );
      }
    ...
    error: function (xhr, textStatus, error) {
      ...
      frappe.msgprint(
        '<div style="color: red;">Error occurred while importing data: <br>' +
          error +
          "</div>"
      );
    },
      ...
      var startDialogFields = [
        {
          ...
          fieldtype: "HTML",
          options:
            "Currently Applied Filters, These will be used for importing",
          read_only: 1,
        },
      ];

      filterTemplate.forEach((value) => {
        startDialogFields.push({
          ...value,
          default: filterValues[value.fieldname],
          read_only: 1,
        });
      });
  ...
  report.page.add_inner_button(__("Send Marks"), () => {
    ...
    let dialog = new frappe.ui.Dialog({
      ...
      primary_action: async function () {
        dialog.hide();
        let print_format = await frappe.db.get_value(
          "Assessment Group",
          assessment_group,
          "result_print_format"
        );
        if (!print_format.message.result_print_format) {
          frappe.msgprint({
            ...
            message: __(
              "Result Print Format must be present in Assessment Group"
            ),
            indicator: "orange",
            primary_action: {
              label: "Okay",
          ...
          frappe.confirm(
            __("Are you sure you want to send marks?"),
            () => {
...
function goToProcessing() {
  ...
  frappe.set_route("process-exam-result", {
    academic_year,
    school,
    program,
    exam,
...
function addNote() {
  const noteContainer = frappe.query_report.parent.querySelector(
    ".page-head .container"
  );
  ...
  noteContainerDiv.classList.add(
    "note-container",
    "blue",
    "form-message",
    "my-0"
