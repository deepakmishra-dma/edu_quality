...
def get_columns(assessment_group, filters):
	...
	if assess_group.is_group or assess_group.custom_is_composite:
		frappe.throw("Can't enter marks of a assessment group of group type or composite type")
...
def get_composite_exam_columns(assess_group, filters):
	...
	for atom_assess_group in assess_group.get("custom_composite_exams"):
		columns = [
			*columns,
			*get_subject_criteria_columns({"name": atom_assess_group.assesment_group}, filters),
		]
	return columns
...
def get_desc_earlier_marks(filters, students, return_hash=False):
	...
	sorted_data = sorted(final_data, key=lambda x: (x.get("assess_plan_order"), x.get("order")))
...
def get_earlier_marks(filters, students, criterias):
	...
	for student in students:
		...
		for assess_res in students_res[curr_ref]:
			...
			student[gen_field_name(assess_plan)] = parse_score(is_absent, scoring_type, score, grade)
...
def do_mark_entry(data, filters):
	...
	assessment_group_doc = frappe.get_cached_doc("Assessment Group", filters.get("assessment_group"))
...
def add_assessment_criteria(assessment_details, criteria, is_descriptive, is_remarks=None):
	...
	if str(score).lower() == "-" or score is None:
		score = 0
		is_absent = 1
...
def append_assessment_criteria(assessment_details, criteria, is_descriptive=None, is_remarks=None):
	"""Dedupes similar assessment criteria and modifies instead of adding if it already exists"""

	all_criterias = [gen_desc_assessment_key(i, is_descriptive, is_remarks) for i in assessment_details]

	if gen_desc_assessment_key(criteria, is_descriptive) in all_criterias:
		index = all_criterias.index(gen_desc_assessment_key(criteria, is_descriptive, is_remarks))
...
def export_custom_csv(report_name="Marks Entry Tool", filters=[]):
	...
	if not data.columns:
		...
		return
	data.columns = [{**datum, "label": datum.get("fieldname")} for datum in data.columns]
...
def import_csv_background(csv_content, filters):
	...
	try:
		...
		frappe.enqueue(do_mark_entry, data=data, filters=filters, queue="long", timeout=1800)
