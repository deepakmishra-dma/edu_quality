...
def get_data(filters):
	...
	query = (
		...
		.where((cmap_table.academic_year == academic_year) & (cmap_assign_table.school == school))
	...
	final_query = (
		frappe.qb.from_(left_query).where(left_query.teacher.isnull()).select(left_query.name.as_("teacher"))
	)
