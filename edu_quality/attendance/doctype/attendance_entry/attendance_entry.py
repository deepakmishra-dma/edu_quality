...
class AttendanceEntry(Document):
	...
	def is_early_pickup(self):
		...
		timestamps = frappe.get_all(
			"Absent and Delay", {"parent": self.name, "status": ["like", "%early%"]}, pluck="timestamp"
		)
	...
	def send_notification_to_councellor(self):
		...
		try:
			...
			frappe.log_error("Error Sending Notification to Councellor", frappe.get_traceback())
