...
def before_save(self, method=None):
	...
	if old_doc and old_doc.workflow_state != "Received" and self.workflow_state == "Received":
		...
		for item in id_card_items:
			if not frappe.db.get_value("Purchase Order", item.purchase_order, "custom_is_id_card"):
				continue
			...
			id_card_doc = frappe.get_doc("Permanent Id Card", reference_docname, ignore_permissions=True)
