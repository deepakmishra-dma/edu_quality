...
from edu_quality.public.py.utils import gen_qr_code_b64, im_2_b64
...
def transform_data(items, selected_items, purchase_receipt_items=None, self=None):
	...
	if self.get("custom_is_id_card"):
		perm_id_card = frappe.get_doc("Permanent Id Card", self.get("custom_reference_docname"))
		item_code_data["ID Card"]["custom_product_url"] = perm_id_card.file
	for item in items:
		school_name = warehouse_to_name_map.get(item.get("warehouse")) or item.get("warehouse")
		if not item_map.get(item.get("item_code"), False):
			item_map[item.get("item_code")] = {
				...
				"receipt_created": (
					purchase_receipt_items.get(item.get("item_code")) if purchase_receipt_items else False
				),
			...
			last_qty_of_school = item_map.get(item.get("item_code"), {}).get(school_name, 0) or 0
			total_qty = item_map.get(item.get("item_code"), {}).get("total_qty", 0) or 0

			item_map[item.get("item_code")][school_name] = last_qty_of_school + item.get("qty", 0)

			item_map[item.get("item_code")]["total_qty"] = total_qty + item.get("qty", 0)
	selected_items_hash = {}
	if selected_items:
	...
	transformed_items = [
		...
		if selected_items_hash.get(item) or selected_items is None or not selected_items
	]
...
def generate_challan_list(self, selected_items=None):
	...
	if len(purchase_receipt_items):
		purchase_receipt_items = {i.get("item_code"): True for i in purchase_receipt_items}
	else:
		purchase_receipt_items = None
	self = transform_data(self.get("items"), selected_items, purchase_receipt_items, self)
...
def create_purchase_receipt(self, school, selected_items=[]):
	...
	filtered_items = list(
		filter(
			...
			and f"{item.get('item_code')}-{item.get('warehouse')}" not in warehouse_to_item_map
			and (
