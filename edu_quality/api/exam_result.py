...
from frappe.query_builder.functions import GROUP_CONCAT, Count, Sum
...
def get_assessment_result_of_plans(assessment_plans, docstatus=[0, 1], student_master=None):
...
def diff_students_and_results(students, results, assessment_plan):
	...
	for student in students:
		if student not in result_hash and student not in no_data_students:
			errors.append(f"No Result Created for student {student} for assessment_plan {assessment_plan}")
...
def _process_atomic_exam(
	...
	process_toppers(assessment_group, assessment_plans, get_division_set(assessment_plans))

	frappe.msgprint("Successfully Processed all the results matching the criteria provided")
...
def get_subject_set(assessment_plans):
	return set(
		plan.get("course") for plan in assessment_plans if plan.get("custom_scoring_type").lower() == "marks"
	)


def process_assessment_results(assessment_plans, student_master):
	submitted_docs = get_assessment_result_of_plans(assessment_plans, [1], student_master)
	cancel_submitted_atomic_exams(submitted_docs)
	non_submitted_docs = get_assessment_result_of_plans(assessment_plans, [0], student_master)
...
def handle_error():
	...
	frappe.msgprint("Error in processing atomic exam. Please check the logs for more details")
...
def cancel_submitted_atomic_exams(result_data):
	submitted_results = [result.get("name") for result in result_data if result.get("docstatus") == 1]
...
def get_result_from_plans(assessment_plans, group_by=False, docstatus=[0, 1], student_master=None):
	assess_res_qb = frappe.qb.DocType("Assessment Result")
	assess_res_de_qb = frappe.qb.DocType("Assessment Result Detail")
	student_con = get_student_con(student_master, assess_res_qb)
	plans = [plan.get("name") for plan in assessment_plans]
	...
	if group_by:
		...
		query = (
			...
			.select(
				...
				Sum(assess_res_de_qb.custom_processed_result * assess_group_qb.custom_scale).as_("score"),
...
def process_composite_result(assessment_group, academic_year, program, div=None, student_master=None):
	...
	for atomic_exam in composite_exams:
		process_atomic_exam(atomic_exam.name, academic_year, program, div, student_master)
	...
	for result in all_results:
		...
		for parent in modified_result:
			if modified_result[parent]:
				assess_result = frappe.get_cached_doc("Composite Assessment Result", parent)
				combined_marks_or_grade = 0
