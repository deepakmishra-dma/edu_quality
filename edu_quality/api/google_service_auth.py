...
class GoogleServiceAccountAuth:
	...
	def get_google_service_object(self, imporsonate_user=None, imporsonate=False):
		...
		if imporsonate:
			...
			credentials = service_account.Credentials.from_service_account_file(
				get_absolute_path(self.google_settings.service_account_credentials_json), scopes={self.scopes}
			)
	...
	def create_meet(self, summary, description, start_time, duration_minutes, imporsonate_user):
		...
		event = (
			service.events()
			.insert(calendarId="primary", body=event, sendNotifications=True, conferenceDataVersion=1)
			.execute()
		)
	...
	def list_recordings(self):
		...
		recordings = (
			service.drive()
			.files()
			.list(q="mimeType='application/vnd.google-apps.folder' and name='Meet Recordings'")
			.execute()
		)
	...
	def get_meeting_details(self, event_id):
		...
		recording_link = None
		recording_info = (
			service.events().instances(calendarId="primary", eventId=event_id).execute().get("items", [])
		)
		print(recording_info)
		if recording_info:
			recording_link = recording_info[0].get("hangoutLink")

		meeting_details = {"participants": participants, "recording_link": recording_link}
