...
def change_image_bg(image_path, bg_color):
	...
	new_image_path = new_image_path.relative_to(Path(frappe.get_site_path() + "/public"))
...
def generate(**kwargs):
	...
	program_enrollment = [
		frappe.get_doc("Program Enrollment", enrollment) for enrollment in kwargs.get("enrollments")
	]
...
def generate_permanent_id_cards(enrollments):
	...
	for i in program_enrollment:
		...
		if i.get("custom_status") in STATUSES or student_status in STATUSES:
			frappe.msgprint("ga")
			return frappe.throw(f"Cannot Create ID Card for cancelled student or alumni students {name}")
		if not hash:
			hash = i
			...
			return frappe.throw(f"Academic Year is not same for all selected, in {name}")
	frappe.enqueue(generate_permanent_id_cards_async, enrollments=enrollments, queue="long")
