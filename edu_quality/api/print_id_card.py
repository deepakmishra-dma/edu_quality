...
def change_image_bg(image_path, bg_color):
    ...
    im.convert("RGB" if original_mode in ('RGBA', 'RGB', 'LA') else original_mode).save(new_image_path, "JPEG")
...
def generate_permanent_id_cards(**kwargs):
    ...
    template = frappe.render_template(
        ...
        {"program_enrollments": enrollment_in_chunks, "background_images": background_images},
    )
    html = HTML(string=template, base_url=base_url)
    main_doc = html.render()
    main_pdf = main_doc.write_pdf()
