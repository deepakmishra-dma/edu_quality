...
def bundle_all_cards(doc):
	...
	query = (
		...
		.where((ind_perm_card_doc.parent == doc.name) & (ind_perm_card_doc.parenttype == "Permanent ID Card"))
...
def create_purchase_order(name):
	try:
		if not frappe.db.exists("Permanent Id Card", name):
			frappe.throw("Doesn't exist, the specified pdf for the name provided doesn't exist")
