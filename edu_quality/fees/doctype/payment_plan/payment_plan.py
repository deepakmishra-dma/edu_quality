...
class PaymentPlan(Document):
	...
	def validate_existing_payment_plan(self):
		existing_payment_plan = frappe.db.get_all(
			"Payment Plan",
			{
			...
			for plan in existing_payment_plan:
				if self.get_doc_before_save() and plan.name == self.name:
					...
					if len(existing_schedule) == len(self.payment_schedule):
						...
						is_identical = all(
							term.payment_term == existing_term.payment_term
							and term.invoice_portion == existing_term.invoice_portion
							for term, existing_term in zip(
								self.payment_schedule, existing_schedule, strict=False
							)
						)
						if is_identical:
