...
def get_breakup(fees, term):
	...
	for component in fees.components:
		...
		if frappe.db.exists("Fee Category", component.fees_category):
			display_name = (
				frappe.db.get_value("Fee Category", component.fees_category, "display_name")
				or component.fees_category
			)
		breakup = [
			{
...
def get_payment_details(**kwargs):
	...
	if payment_request.payment_term and frappe.db.exists(
		"Payment Request",
		[
			["Payment Request", "status", "=", "Initiated"],
			["Payment Request", "payment_term", "is", "not set"],
			["Payment Request", "reference_name", "=", payment_request.reference_name],
		],
	):
		request = frappe.db.get_value(
			"Payment Request",
	...
	if payment_request.payment_term:
		...
		if previous_request:
			return {
				"redirect": frappe.utils.get_url()
				+ "/payment?payment_request="
				+ previous_request.payment_hash
			}
		...
		if fees.doctype == "Fees":
			due_date = frappe.get_value(
				"Payment Schedule",
				{"parent": fees.name, "payment_term": payment_request.payment_term},
				"due_date",
			)
		...
		for fee in fees.components:
			...
			if fee_type != "Regular":
				...
				if frappe.db.exists("Fee Category", fee.fees_category):
					display_name = (
						frappe.db.get_value("Fee Category", fee.fees_category, "display_name")
						or fee.fees_category
					)
				breakup.append(
					{
						"fees_category": display_name,
...
def get_discounts(fees):
	...
	return {
		...
		"referral_discount_company": referral_discount_company,
		"other_discount_company": other_discount_company,
	}
...
def payment_charge(**kwargs):
	...
	frappe.response["message"] = {
		"charge": charge,
		"url": payment_request.get_payment_url(payment_method=kwargs.get("pm")),
	}
...
def payment_receipt(payment_request, category):
	try:
		...
		payment_entry = frappe.get_value(
			"Payment Entry", {"payment_request": payment_request, "company": company}, ["name"]
		)
...
def get_company(payment_request, category):
	...
	doctype, fee_name = frappe.get_value(
		"Payment Request", payment_request, ["reference_doctype", "reference_name"]
	)
	fee = frappe.get_doc(doctype, fee_name)
	company = next(
		(component.custom_company for component in fee.components if component.fees_category == category),
		None,
	)
...
def get_last_unpaid_request(payment_request):
	...
	payment_schedule = frappe.get_all("Payment Schedule", filters, fields, order_by="idx")
