...
def execute():
    ...
    process_data(junior_kg_applicants)
    ...
    process_data(senior_kg_applicants)
    ...
    process_data(nursery_applicants)
    
def process_data(applicants):
    for applicant in applicants:
            doc = frappe.get_doc("Student Applicant",applicant.name)
            doc.school = school_map[doc.school]
            doc.program = "Junior KG-" + doc.school
            doc.fee_structure = ""
