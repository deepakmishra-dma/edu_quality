...
from edu_quality.public.py.utils import get_mobile_number, sms_otp, verify_otp
...
def generate_undertaking_otp(payment_hash=None, fee=None, fee_advance=None):
	if not trigger_event:
		return False
	doctype, docname = (
		("Fees", fee) if fee else ("Fee Advance", fee_advance) if fee_advance else get_fee(payment_hash)
	)
...
def verify_undertaking_otp(otp, payment_hash=None, fee=None, fee_advance=None):
	doctype, docname = (
		(fee and ("Fees", fee)) or (fee_advance and ("Fee Advance", fee_advance)) or get_fee(payment_hash)
	)
