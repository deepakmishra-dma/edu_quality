...
def update_component(
	...
	if is_amount and fees.doctype == "Fee Advance":
		...
		invoice_portion = frappe.get_value(
			"Payment Schedule",
			{"payment_term": fees.payment_term, "parent": fees.payment_plan},
			"invoice_portion",
		)
...
def remove_and_update_component(
	component_name,
	discount_name,
	discount,
	discounted_amount,
...
def get_label(fee_category):
	...
	if label:
		...
		frappe.throw(f"Please set custom label for fee category {fee_category}")
	return label
		...
		if frappe.db.exists("Discount Configuration", dis_filter):
			...
			if dis.discount_amount:
				...
				component.custom_discount_percentage = calculate_discount(
					component.amount, dis.discount_amount
				)
...
def update_breakups(dis, component, fees, term="All", update=0, remove=0, custom=0):
	...
	try:
		...
		if not dis.discount_amount and dis.discount is not None:
			dis.discount_amount = flt(dis.discount * component.amount / 100, 2)
		# update component

		discount_breakup = update_discount_breakup(
			component.amount, component.discount_breakup, dis.discount, dis.discount_amount, dis.name, remove
		)
		if not update:
...
def update_discount_breakup(
	component_amount, discount_breakup, discount, discount_amount, discount_name, remove
):
	"""
		...
		for dis in breakup:
			...
			for schedule in fees.payment_schedule:
				if term == schedule.payment_term:
					...
					discount = flt((discount_amount / schedule.payment_amount) * 100, 2)
					new_breakup = update_discount_breakup(
						schedule.payment_amount, schedule.discount_breakup, discount, discount_amount, dis, 0
					)
