...
def create_fees(doc, method=None):
    try:
        ...
        existing_pe = frappe.get_all("Program Enrollment", {"student": doc.student})
        if student.imported and student.student_status not in ["New student", "Cancelled"] and len(existing_pe) <= 1:
            return
