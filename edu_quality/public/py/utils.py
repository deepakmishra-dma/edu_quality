...
def gen_qr_code_b64(input_str: str | bytes) -> str:
...
def academic_year_query(
	...
	return (
		...
		.where(
			(acad_year_qb.custom_current_academic_year == 1) | (acad_year_qb.custom_next_academic_year == 1)
		)
		.select("name")
	).run(as_dict=as_dict)
...
def split_desc_hash(data):
	...
	for result_key in data:
		...
		if subject not in subject_question:
