...
function checkDuplicates(frm, cdt, cdn) {
  ...
  frm.doc.assessment_criteria.forEach(function (row, i) {
    if (
      ...
      row.custom_exam_type === d.custom_exam_type &&
      row.name != d.name
    ) {
      frappe.msgprint(
        `Assesment Criteria/Exam Component Combination already exists for ${row.assessment_criteria}, ${row.custom_exam_type}`
      );
...
async function get_textbooks(frm) {
  const res = await frappe.call({
    method:
      "edu_quality.edu_quality.overrides.assessment_plan.get_assessment_cr_textbooks",
  });
...
function addFromDescriptiveExam(frm) {
  if (!frm.doc.custom_is_descriptive || frm.doc.docstatus !== 0) {
    return;
  }
  ...
  const method =
    "edu_quality.edu_quality.overrides.assessment_plan.get_questions";
...
function addRemarks(frm) {
  ...
  const method =
    "edu_quality.edu_quality.overrides.assessment_plan.get_remarks";
