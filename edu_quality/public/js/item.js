async function getItemCode(frm) {
  ...
  if (
    !frm.doc.custom_subject ||
    !frm.doc.custom_class ||
    !frm.doc.custom_textbook ||
    !frm.doc.custom_chapter ||
    !frm.doc.item_group ||
    !frm.doc.custom_is_cmap
  )
    return;
...
function uploadFileButton(frm) {
  frm.get_field("custom_upload_file_to_drive").onclick = async function () {
    if (frm.doc.__islocal) {
      frappe.msgprint({
        message: __(
          "Please Save the item before uploading product document on drive"
        ),
        indicator: "red",
        title: __("Error"),
      });
    ...
    const allowed_extensions =
      item_group_doc?.message?.custom_allowed_file_extensions
        ?.split(",")
    ...
    input.onchange = function () {
      ...
      if (file) {
        ...
        if (allowed_extensions.includes(ext)) {
          ...
          fetch(
            "/api/method/edu_quality.overrides_hooks.item.upload_to_drive",
            {
              method: "POST",
              body: form_data,
            }
          ...
          frappe.msgprint(
            __(
              `Please upload only ${allowed_extensions
                ?.map((ext) => `.${ext}`)
                .join(",")} files.`
            )
          );
...
function appendUrl(frm) {
  $(frm.fields_dict.custom_product_url.$wrapper)
    .find("label")
    .html(
      `<a target="__blank" href="${frm.doc.custom_product_url}">Product URL <i class="fa fa-external-link" aria-hidden="true"></i></a>`
    );
}
  ...
  refresh: function (frm) {
    ...
    frm.get_field("custom_view_worksheet_header").onclick = function () {
      if (frm.doc.__islocal) {
        frappe.msgprint({
          message: __(
            "Please Save the item before accessing the worksheet header"
          ),
          indicator: "red",
          title: __("Error getting worksheet header"),
        });
      ...
      if (!frm.doc.__islocal) {
        window.open(
          `/api/method/edu_quality.overrides_hooks.item.get_worksheet_template?name=${frm.doc.name}`
        );
      }
    };
  },
  ...
  frappe.call({
    ...
    callback: function (response) {
      ...
      if (doc.custom_printable == 0) {
        ...
        $(frm.fields_dict.custom_print_ready.$wrapper).css(
          "display",
          "inline-block"
        );
      }
