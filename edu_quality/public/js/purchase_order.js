...
function generateOrderCard(
  ...
  return `<div
                  style="
                    ...
                    box-shadow: none;
                    background-color: ${
                      item_created ? "#f0fdf4" : "var(--card-bg)"
                    };
                    padding:8px 12px;
                  "
                >
                ...
                <div style="display:flex;gap:8px;">
                  ...
                  <div style="margin-top:8px;margin-bottom:8px;display:flex; gap:12px;"><div><a href="${productURL}" target="__blank"><i class="fa fa-file" style="color:rgb(29 78 216);font-size:22px" aria-hidden="true" ></i></a></div>${
    item_created
      ? `<button data-is-print="true" data-item-code="${itemCode}" onclick=""><i style="color:rgb(29 78 216);font-size:22px" aria-hidden="true" class="fa fa-qrcode"  data-is-print="true" data-item-code="${itemCode}" aria-hidden="true"></i></button>`
      : ""
  }</div>
                  <div style="display:flex; align-items:center;gap:4px;">
                  ${
                    frm.doc.custom_is_cmap_print
                      ? `<div style="margin-top:4px;">Printed</div><input data-is-check="true" data-item-code="${itemCode}"  ${
                          item_created ? "disabled" : ""
                        } ${
                          item_created
                            ? `checked="${item_created}"`
                            : printed
                            ? `checked="${printed}"`
                            : ""
                        }}  type="checkbox" style="width:22px !important;height:22px;flex-shrink:0;margin-top:8px;" ></input></div>`
                      : ""
                  }
                  </div></div>
                </div>`;
...
function getCheckedItems(frm) {
  const inputsList = Array.from(
    frm.fields_dict.custom_challan_detail.wrapper.querySelectorAll(
...
async function createIdCard(frm) {
  var d = new frappe.ui.Dialog({
    ...
    fields: [
      {
        label:
          "This Will Create Challan for the following selected items across all schools.",
    ...
    async primary_action(values) {
      ...
      frappe.call({
        method:
          "edu_quality.overrides_hooks.purchase_order.create_purchase_receipt_for_all_schools",
        args: {
          self: frm.doc,
...
async function handleCreateCmapReceipt(frm) {
  ...
  setTimeout(() => {
    frappe.call({
      method:
        "edu_quality.overrides_hooks.purchase_order.generate_challan_list",
      args: {
        self: frm.doc,
        selected_items: checkedItems,
      },
      callback: function (r) {
        if (r.message) {
          ...
          const datatable = new DataTable(d.fields_dict.challan_table.wrapper, {
            columns,
            data,
          });
        }
...
function createReceiptButton(frm) {
  ...
  if (
    frappe.user_roles.includes("Printer") ||
    frappe.user_roles.includes("Administrator") ||
    frappe.user_roles.includes("School Admin") ||
    frappe.user_roles.includes("System_Manager")
  )
    frm.add_custom_button(__("Create Challans"), () => {
      handleCreateChallan(frm);
    });
}
...
function addScanBtnForPrinter(frm) {
  ...
  if (
    ...
    frm.add_custom_button(__("Scan Item Code"), async function () {
      ...
      frappe.call({
        method:
          "edu_quality.overrides_hooks.purchase_order.mark_item_as_printed",
        args: {
        ...
        setTimeout(() => {
          ...
          el.addEventListener("click", (e) => {
            ...
            if (dataset.isPrint) {
              frappe.call({
                method:
                  "edu_quality.overrides_hooks.purchase_order.get_linked_receipts",
                args: {
                  name: frm.doc.name,
