frappe.listview_settings["Program Enrollment"] = {
  onload: function (list_view) {
      ...
      try {
        ...
        if (generated?.message === "Enqueued Successfully")
          frappe.msgprint({
            ...
            message: __(
              'Permanent ID Cards Generation Scheduled Successfully. <a href="/app/permanent-id-card" target="_blank">Click Here</a>'
            ),
            primary_action: {
              label: __("OK"),
              action: function () {
    ...
    list_view.page.add_action_item("Generate Temporary ID Cards", async () => {
      ...
      try {
        ...
        const generated = await fetch(
          `/api/method/edu_quality.api.print_id_card.generate`,
          {
            method: "POST",
    ...
    list_view.page.add_action_item("Create Birthday Cards", async () => {
      ...
      frappe.call({
        method:
          "edu_quality.edu_quality.doctype.birthday_card.birthday_card.create_birthday_card",
        args: { program_enrollments: selectedEnrollments },
        callback: function (r) {
          frappe.msgprint({
            title: __("Birthday Cards Creation"),
            message: __(
              'Birthday Cards Creation Scheduled Successfully. <a href="/app/birthday-card" target="_blank">Click Here</a>'
            ),
            primary_action: {
              label: __("OK"),
              action: function () {
    ...
    frappe.db
      .get_list("Academic Year", {
        filters: { custom_current_academic_year: 1 },
        fields: ["name"],
      })
      .then((response) => {
        if (!frappe.route_options) {
          const result = response?.[0]?.name;
          frappe.listview_settings["Program Enrollment"]["filters"] = [
            ["academic_year", "=", result],
          ];
        }
      });
  },
