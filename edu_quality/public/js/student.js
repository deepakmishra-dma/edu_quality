            ...
            .map((item, index) => {
              ...
              return `
                            <tr>
                                ...
                                <td>${
                                  outstanding === 0
                                    ? formatDate(paid_date, "paid_date")
                                    : "Not Paid"
                                }</td>
...
function swapDivisionButton(frm) {
  frm.add_custom_button(
    __("Swap/Change Division"),
    async function () {
      let cur_ay = await frappe.db.get_value(
        "Academic Year",
        { custom_current_academic_year: 1 },
        ["name"]
      );
      let cur_pe = await frappe.db.get_value(
        "Program Enrollment",
        {
          student: frm.doc.name,
          program: frm.doc.program,
          academic_year: cur_ay.name,
          docstatus: 1,
        },
        ["name", "student_group"]
      );
      if (!cur_pe.message || Object.keys(cur_pe.message).length === 0) {
        frappe.msgprint({
          ...
          message: __(
            "Program Enrollment not found for this student! for current academic year. Please check and try again."
          ),
          primary_action: {
            label: __("OK"),
            action: function () {
      ...
      let cur_batch = await frappe.db.get_value(
        "Student Group",
        { name: cur_pe.message.student_group },
        "batch"
      );
      let d = new frappe.ui.Dialog({
          ...
          },
          {
            ...
            get_query: function () {
              return {
                ...
                filters: [
                  ["program", "=", frm.doc.program],
                  ["academic_year", "=", cur_ay.message.name],
                  ["name", "!=", cur_pe.message.student_group],
                  ["disabled", "=", 0],
                ],
              };
        ...
        primary_action: async function (values) {
          let new_batch = await frappe.db.get_value(
            "Student Group",
          ...
          if (cur_batch.message.batch == new_batch.message.batch) {
            ...
            frappe.confirm(
              "Change in batches, Are you sure you want to proceed?",
              () => {
                swapDivision(
                  frm,
                  values.student,
          ...
          },
          {
            ...
            get_query: function () {
              return {
                doctype: "Student Group",
                filters: [
                  ["program", "=", d.get_value("class")],
                  ["academic_year", "=", frm.doc.custom_academic_year],
                ],
              };
            },
          },
