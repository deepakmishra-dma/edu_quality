frappe.ui.form.on("Employee", {
  refresh: async function (frm) {
      ...
      frm.add_custom_button(__("Mark Ex Employee"), function () {
        frappe.confirm(
          ...
          () => {
            frappe.call({
              method:
                "edu_quality.edu_quality.server_scripts.employee.mark_ex_employee",
              type: "POST",
              args: {
    ...
    if (frm.doc.status == "Left" && frm.doc.is_migrated == 0) {
      let school = await frappe.db.get_value(
        "Instructor",
        { employee: frm.doc.name },
        "custom_school"
      );
      frm.add_custom_button(__("Migrate Data"), function () {
        let d = new frappe.ui.Dialog({
          ...
          fields: [
            {
              ...
              get_query: function () {
                return {
                  query:
                    "edu_quality.edu_quality.server_scripts.employee.employee_query",
                  filters: {
                    school: school.message.custom_school,
                    status: "Active",
                  },
                };
              },
          ...
          primary_action(values) {
            frappe.call({
              method:
                "edu_quality.edu_quality.server_scripts.employee.migrate_employee_data",
              type: "POST",
              args: {
                employee: frm.doc.name,
