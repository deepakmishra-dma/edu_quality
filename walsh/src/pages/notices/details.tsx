...
export const NoticeDetails: React.FC = () => {
  ...
  const useFilteredNotices = (list: any, searchQuery: any) => {
    if (!list) return [];
    if (!searchQuery) return list?.data?.message || [];
    return list?.data?.message?.filter((item: any) => {
  ...
  // Inside your NoticeDetails component
  const filteredList = useFilteredNotices(list, searchQuery);
  ...
  return <Text align="center" color="dimmed" weight="bold" my={30}>
    No Notice Found
  </Text>
};
