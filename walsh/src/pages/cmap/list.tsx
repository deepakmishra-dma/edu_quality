...
const CmapList = () => {
  ...
  const { data: cmapList, isLoading } = useCmapList(
    subject,
    unit,
  ...
  const subjectTitle = classDetails?.data?.message?.class?.subject?.find(
    (c) => c.subject === searchParams.get("subject")
  )?.subject;
  ...
  return (
    <Box>
      ...
      >
        <Text>
          {subjectTitle}: Unit {searchParams.get("unit")}
        </Text>
      </Box>
      <Box
        sx={{
      ...
      >
        ...
        {cmapList?.data?.message?.map((cmap, i) => {
          ...
          const broadCasts = Array.from(
            new Set(
              cmap?.products?.map((product) => product?.broadcast_description)
            )
          ).filter(Boolean);
          const homeWorks = Array.from(
            new Set(
              cmap?.products?.map((product) => product?.homework_description)
            )
          ).filter(Boolean);
          const parentNotes = Array.from(
            new Set(
              cmap?.products?.map((product) => product?.parentnote_description)
            )
          ).filter(Boolean);
                  ...
                  {products.map((product, i) => {
                    ...
                    return (
                      ...
                      >
                        {isOpen && (
                          <a
                            href={product?.item_data?.custom_product_url}
                            onClick={(e) => {
                              e.stopPropagation();
                            }}
                          >
